...
| R-04 | Topmost MenuHost popup lingers over Alt+Tab | High | Medium | Keep no-activate style and Alt/foreground timer | Apple/Control open then Alt+Tab screenshot | Apple PASS; Control OPEN |
...
| R-07 | YAML/schema edit blanks toolbar | High | Medium | Stop Seelen before edit; parser/schema-aware diff; log check after restart | Seelen `SerdeYaml`/panic scan and screenshot | No Seelen edit planned |
| R-08 | Dock covers maximized content | High | Medium | Seelen WEG sole ownership; no MenuHost appbar; per-monitor work-area checks | Compare client/window bounds with monitor work area; screenshots | Baseline PASS |
...
| R-12 | Repeated panels leak handles/memory | High | High for Control Center churn | Profile/dispose form, timer, async probe, child-process, and COM lifetimes; longer soak test | Isolated Apple/Control working-set and handle samples after settle | Confirmed: Control-only 10 cycles retained +68 handles |
...
| R-16 | Live/repo WEG drift breaks pins after app updates | Medium | High | Prefer UMIDs or path-aware restore; report drift before copy | no-index diff; launch every restored pin | Drift observed; Codex icon log error |
