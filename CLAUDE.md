...
- UX model for the top bar: informational CPU, RAM, and NET readouts live in the CENTER cluster and are not clickable. Network, Bluetooth, the merged battery/charging readout, Control Center, calendar, and notifications live on the RIGHT and each opens its intended surface. Never remove the center telemetry cluster or give several right-side icons the same target without explicit user approval.
