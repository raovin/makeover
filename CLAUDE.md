...
- Do not wire the APPLE toolbar `onClick` to a URI; Apple clicks stay helper-routed (the Apple zone is left-anchored and never drifts, and the helper path is instant). Do not point any `macmakeover-*:` handler at a PowerShell/WPF cold chain - that path was measured as visibly laggy. `verify.ps1` enforces both.
