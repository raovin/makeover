...
- Keep the Control Center protocol registered to `conhost.exe --headless` running `scripts/Show-MacControlCenter.ps1` via `scripts/Install-MacControlCenterHandler.ps1`; the top-right network, sliders, and power/battery widgets should open the custom MenuHost popover, not Seelen's built-in quick-settings/power flyout. This protocol is fallback plumbing only; normal Control Center clicks are routed by `scripts/start-hot-corners.ps1` to the resident .NET MenuHost.
